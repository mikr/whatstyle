...
impl RasterizerIntersection for Rasterizer {
      ...
      for edge in &scene.edges {
         match edge.edge_type {
            EdgeType::LTR | EdgeType::LTL | EdgeType::LBR | EdgeType::LBL => {
               ...
               let (vert_end, x_first_px) =
                  v_multi_intersect_fast(p1,
                                         p2,
                                         self.div_per_pixel,
                                         vert_ref.start,
                                         &mut self.vert_intersections);

               let (hori_end, y_first_px) =
                  h_multi_intersect_fast(p1,
                                         p2,
                                         self.div_per_pixel,
                                         hori_ref.start,
                                         &mut self.hori_intersections);
   ...
   fn h_intersection(&self, edge: &Edge, y_px: i64) -> i64 {
      ...
      self.hori_intersections[h_ref.start + (y_px - h_ref.first_px) as usize]
   }
   ...
   fn v_intersection(&self, edge: &Edge, x_px: i64) -> i64 {
      ...
      self.vert_intersections[v_ref.start + (x_px - v_ref.first_px) as usize]
   }
}


fn h_multi_intersect_fast(p1: &Point,
                          p2: &Point,
                          step_y: i64,
                          mut vec_start: usize,
                          inters: &mut Vec<i64>)
...
fn v_multi_intersect_fast(p1: &Point,
                          p2: &Point,
                          step_x: i64,
                          mut vec_start: usize,
                          inters: &mut Vec<i64>)
